...
namespace FlatBuffersSetup.Scripts.SettingsImport.Editor
{
    public abstract class ProjectImporter : GoogleSheetsImporter
    {
        ...
        public void SaveToFile() => SettingsFileUtils.SaveSettings(LocalSettings, ImportConstants.SETTINGS_FOLDER, SettingsFileName);
        
        public void LoadFromFile() => LocalSettings = SettingsFileUtils.LoadSettings(LocalSettingsFilePath);

        protected Task DownloadAndParseSheet() => DownloadAndParseSheet(SheetName);
    }
}